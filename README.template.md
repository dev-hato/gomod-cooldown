...
Go ${GO_VERSION} or later is required to build or install `gomod-cooldown`.
...
being upgraded. A newer module may have removed one of those internal
packages. If so, the upgrade can fail with "does not contain package" even
though the main module does not import that package directly. Scoping the
update to `./...` starts from packages in the main module instead.
...
- Otherwise, a normally exiting child's status is returned unchanged. On
  ...
  temporarily owns the foreground while it runs. As a result, terminal input
  and terminal-generated SIGINT and SIGTERM retain their normal behavior.
  Signal termination is reported as `128 + signal` (`130` for SIGINT and `143`
  for SIGTERM).
...
Also, cooldown can exclude the highest usable compatible version named by the
raw list. Even then, implicit discovery does not select a higher
`+incompatible` version for that reason alone. If the removed compatible
version has a real, module-aware `.mod` file, higher `+incompatible`
candidates are omitted too. A synthetic legacy `.mod` containing only
`module <path>` is not treated as evidence of module awareness, so those
...
`.info.Time` is a commit time, not a publish time. A new tag can point at an
old commit. This is the default availability source because it only requires
...
cutoff through the present using `since` and `limit=2000`, paging until the
final short page. It can take a long time for a large cooldown window. A
malformed record, HTTP failure, timeout, or cursor that does not advance fails
setup closed; it never silently falls back to commit time. This mode requires
the exact official `https://proxy.golang.org` upstream.
...
`@latest`. Meanwhile, a pinned pseudo-version remains downloadable via its
...
- The tool cannot resolve an ambiguous import caused by the same package being
  ...
  retrying.
- An exact request such as `go get example.com/mod@v1.2.3` uses
...
- A 502 means discovery metadata or the complete index snapshot could not be
  ...
  empty list.
- Excluded versions are logged. The logged fields are module, version, commit
  time, available first-cached time, effective availability time, and cutoff.
- This is not a substitute for Dependabot security updates. It is intended to
  be used alongside them.

## Development

The minimum supported toolchain is Go ${GO_VERSION}. CI runs the core test suite on
Go ${GO_VERSION}, and tests, builds, and smoke-tests the CLI on Linux, macOS, and
Windows. See
...
End-to-end cases run the real Go command against a local fake GOPROXY. They
also exercise byte-for-byte, fixed-commit `go.mod` snapshots from Prometheus,
Helm, and Caddy. Fixture provenance is recorded in
...
The v1 compatibility contract covers flag names and meanings, defaults, and
stdout/stderr behavior. It also covers the documented exit codes and the
platform-specific signal behavior documented above. It also covers the
`github.com/dev-hato/gomod-cooldown` install/module path. A corrective patch
that brings filtering behavior back into line with the documented policy may
change an individual version decision. Such changes are documented in
